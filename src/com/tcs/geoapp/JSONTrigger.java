...
public class JSONTrigger extends HttpServlet {
		...
		try {
			...
			if(triggerCount!=0){
				...
				while (rSet.next()) {
					...
					String triggerID = rSet.getString(1);
				...
				response.getWriter().write("<h2>Token id "+ tokenID+ " does not exists!<br><br></h2>"+mainObject.toString());	
			}
		}
		catch (Exception e) {
			e.printStackTrace();
